...
class HWNASBenchAPI():
    ...
    def query_by_index_and_device(self, arch_index, device, dataname):
        if self.search_space == "nasbench201":
            metric_name = f"{device}_latency"
            return float(self.HW_metrics[self.search_space][dataname][metric_name][arch_index])
